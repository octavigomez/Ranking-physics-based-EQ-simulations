...
file_2 = os.path.join(path, "coord_sites_EBSZ.csv") #Coordinates of paleosites
file_3 = os.path.join(path, "paleo_rates_EBSZ.csv") #Paleorates of each paleosite
...
# =============================================================================
            ...
            # Sum_lik_mfd = sum(np.array(Log_lik_mfd)[inf_vals])/len(num_eq[inf_vals])
...
# =============================================================================
            ...
            #Sum_lik_paleo = poisson.logpmf(round(Total_cat), round(Total_paleo))
            Sum_lik_paleo = [poisson.logpmf(round(c), round(p)) for c, p in zip(Num_cat, Num_paleo)]
            Sum_lik_paleo = Sum_lik_paleo/np.log(10)
            Sum_lik_paleo = sum(Sum_lik_paleo)/sum(Num_cat)
...
Sum_lik_model_paleo_perc = [(n-max(Sum_lik_model_paleo))/(min(Sum_lik_model_paleo)-max(Sum_lik_model_paleo)) for n in Sum_lik_model_paleo]
...
Paleo_plot = Sum_lik_model_paleo/sum(Sum_lik_model_paleo)
szs = ((Paleo_plot/min(Paleo_plot))*1.1)**1.7
...
labels_int = [round(a/max(labels_int)) for a in labels_int]
...
fig25, axes =plt.subplots(10,1, figsize = (sizes1, sizes2), dpi = 600, gridspec_kw=gridspec)
...
axs1.text(0.5, -0.015, "r=" + str(r_squared_AM), fontsize = 7.5, horizontalalignment = "left", verticalalignment = "bottom")
...
axs2.text(0.5, -0.015, "r=" + str(r_squared_AD), fontsize = 7.5, horizontalalignment = "left", verticalalignment="bottom")
